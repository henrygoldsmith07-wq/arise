  ...
  return (
    <div className="px-4 pt-5 pb-2 space-y-4 max-w-3xl mx-auto">
      ...
      <div role="search" className="relative">
        ...
        {searchQuery && (
          searchMatches.length ? (
            ...
            <p className="absolute inset-x-0 top-full mt-1 z-20 rounded-xl border border-line bg-surface px-3 py-2.5 text-xs text-ink3 shadow-lg">No settings match “{searchQuery}â€.</p>
          )
        )}
      </div>
      ...
      <section id="sec-gym" className="rounded-2xl border border-line bg-surface p-4 space-y-3">
        ...
        <p className="text-xs text-ink3">Built for one-thumb logging between sets: focus mode, swipe gestures and a load keypad live inside every standard session (the ðŸ‹ï¸ button in the session header).</p>
      ...
      <section id="sec-privacy" className="rounded-2xl border border-line bg-surface p-4 space-y-2">
        ...
        <div className="rounded-xl border border-line bg-surface2 px-3 py-2.5 space-y-2">
          ...
          {store.demo ? (
            <>
              <p className="text-[11px] text-ink3">Sample data is labeled everywhere (banner + demo- prefixed sessions). “Start freshâ€ in the banner wipes it and boots an empty app — export is disabled by design so demo data can never masquerade as yours.</p>
            </>
          ) : (
            <>
